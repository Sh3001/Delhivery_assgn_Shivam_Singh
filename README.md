...
- [Driving the assignment tasks](#driving-the-assignment-tasks)
- [Where to start reading](#where-to-start-reading)
- [System structure](#system-structure)
- [The command pipeline](#the-command-pipeline)
- [The sensor pipeline](#the-sensor-pipeline)
- [The facility](#the-facility)
- [Growing the fleet](#growing-the-fleet)
- [Modules](#modules)
- [Test suite](#test-suite)
- [Configuration guide](#configuration-guide)
- [Goal outcomes](#goal-outcomes)
...
The first line is the default pairing, and it matches the start lanes shipped
in `fleet.yaml` - see **Start lanes** below before inventing your own pairing.
...
### Task 2 - Shared SLAM, map merging, ramp-aware routing

```bash
ros2 launch amr_bringup task2.launch.py rviz:=true goals:=true perfect_localization:=true
```

### Task 3 - Motion shaping, conflict arbitration, safety override
...
### Task 4 - Sensor checking and fleet growth
...
### Task 5 - Build, dependencies, code standards
...
reasoning behind each decision lives in [The command pipeline](#the-command-pipeline)
and [The sensor pipeline](#the-sensor-pipeline) below, and in
...
- **Nav2 never routes around the other robot**, because it cannot see it.
  ...
  as the backstop. This is why start lanes must not aim the robots across each
  other - see *Start lanes*.
- **The filter fails closed.** If a peer has never been located it withholds
  ...
  deadlocking against SLAM - see *Goal outcomes*.
...
*The sensor pipeline*), so Nav2 will never route around the other robot - only the
...
are in *Goal outcomes* below, and the choice is a genuine trade: AMR-1 parks
...
### `Starting point in lethal space` - fixed
...
[Goal outcomes](#goal-outcomes); persistent occurrences from the very
